...
function Login(props) {
  ...
  const showSignUp = () => {
    if (signUp) {
      return <div className = 'account-sign-up'>Already have an account? <a href="#" className = 'purple-text' onClick = {() => {cSignUp(false); resetInput()}}> Sign In</a></div>
    } else {
      return <div className = 'account-sign-up'>Don't have an account? <a href="#" className = 'purple-text'onClick = {() => {cSignUp(true); resetInput()}}> Register</a></div>
    }
  }
    ...
    <div>
      <div className = 'd-flex justify-content-center'>
        <Card className = 'login-card' style = {{maxWidth: '30rem'}}>
          <Card.Header className = 'login-header large-header'>{signUp ? 'Register' : 'Sign In'}</Card.Header>
          <Card.Body>
